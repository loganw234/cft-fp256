...
```bash
node bindings/node/test.mjs         # the package's unit tests, no dependencies
make vectors                        # from the repo root, once
node bindings/node/conformance.mjs  # every published set, replayed twice over
```
...
### Revision 2: thirty-two registers, and the bank as data (ABI 0.9)

docs/SEQUENCER.md's revision 2 added three things on 2026-09-08, and
two of them reach this surface. `r16..r31` need nothing here - a
register is a register, and only an ENCODER has to know that the fifth
bit of each field lives in `imm[27:24]`, which
`bindings/node/seq_corpus.mjs` now does. The **per-run constant bank**
changes the API:

```js
prog.flags;          // the header's flags word
prog.bankExternal;   // PROG_FLAG_BANK_EXT: the image carries NO constants
prog.bankBytes;      // nConsts * the format's element size, or 0

const r = prog.runBank([1.5, 1.25], a, b, c);   // values, or a Uint8Array
prog.digest(bank);   // Uint8Array(32): SHA-256 of image THEN bank
prog.digest();       // the image alone, for a program that carries its own
```

A `BANK_EXT` image is a header and an instruction stream - it is the
SCHEDULE - and `nConsts` says how many constants every run must supply.
One image per positive, loaded once, with the levers and the pass
riding as data.

* **The two run calls refuse each other's programs**, and say which one
  you wanted. A program has ONE source of constants; a bank that was
  silently ignored is how two machines end up computing on different
  numbers while agreeing about the image.
* **The digest covers image AND bank.** Hashing the image alone would
  name the schedule and say nothing about the numbers it ran on. Both
  hashes matter and they are not the same hash: two banks are two
  computations with two digests, and the image's own hash is the same
  in both.
* **A `BANK_EXT` image only loads where the device publishes it**, so
  ask first rather than reading the refusal:
...
`seqFeatures` and the three capacities have been in `cft_caps` since
ABI 0.8 and reached no wrapper until now. The two rules they follow are
different: a clear FEATURE bit means ABSENT, and a zero CAPACITY means
UNKNOWN with nothing enforced against it. `PROG_FLAG_BANK_EXT`,
`SEQ_FEAT_WIDE_CONST`, `SEQ_FEAT_REGS32`, `SEQ_FEAT_BANK_PTR` and
`ALU_EXT_IMUL` are exported by name and checked against the module at
load, because a bit position written down here is a bit position that
can go stale - `CFT_STATUS_DEPOSIT_OVERFLOW` moved once already.

`sha256(bytes)` is the library's own SHA-256, exported for the reason
`cft_sha256` exists: every tool that attests a run wants the same hash
over its own outputs, and four of them had a private copy each.

`node program_test.mjs` is this surface's own test - the recorded
corpus, then programs written by hand for SETACT's early exit, deposit
overflow, five-bit registers and the external bank, then the refusals,
the memory and a negative control. Four of its checks reach
`python/cft_golden/asm.py` through `asm_ref.py` to hold this package's
encoder to the reference in bytes and to run two of the `programs/`
library's images against oracles that are neither; they SKIP by name
where Python or `cft_golden` is out of reach (`CFT_PYTHON` names the
...
seq_corpus.jsonl what libcft's C executor answered for the shared fuzz
                 ...
                 make_seq_corpus.py. REVISION 1: it comes from
                 cft_golden.seq.random_program, which generated
                 revision-1 programs when this corpus was recorded, so
                 revision-2 and -3 programs live in program_test.mjs
                 instead. (seq.py itself has since reached revision 3 -
                 NREG 32, BANK_EXT, the scratch codes - so re-recording
                 the corpus is a choice, not a wait.)
make_seq_corpus.py  the recorder: drives libcft through ctypes, refuses
                 to write anything the golden model disagrees with
asm_ref.py       python/cft_golden/asm.py as a pipe: `.cfta` source in,
                 the reference's own bytes out, so program_test.mjs can
                 hold this package's encoder to it. Optional - the
                 tests that use it SKIP by name without it
remote_test.mjs  the WebSocket transport against the contract, which
