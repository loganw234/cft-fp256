...
```bash
make verify-quick   # ~20 min, 27 of 40 stages: model-vs-C, the bindings,
                    # seven language legs, soak, the five workloads,
                    # the browser demos and the remote backend
make verify-gate    # ~2 h, 35 of 40: the above plus the golden model,
...
reason** rather than passed. `bash verify/run.sh --list` prints all 40 with
