...
the tile was not configured for. Until 2026-09-07 `KMEM_D` was the
odd one out: the header check permitted 256 constants and the tile
STORED only the first sixteen, because no operand field could name
the rest. `kx` closed that gap, and the whole bank is both stored and
addressable now. The software backend enforces none
