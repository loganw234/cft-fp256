...
One thing this rebuild did NOT do, said plainly: the page was not
re-opened in a browser, though its markup changed again. The 0.3
rebuild could not lean on an unchanged template and so was watched in
Chromium; this one has no such watching behind it. What it has instead
is `verify.mjs` step 5, which calls the same eleven wrappers the panel
calls, on the same operands, comparing the same encodings and flags -
a claim about the wrappers, not about the markup that reaches them.
