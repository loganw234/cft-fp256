...
| Python (`cftmpfr`) | complete on `Context` and in `batch`, **755 tests**; `from_str`/`to_str` now go through the library's own conversions, bit-identical to the gmpy2 path they replace on the package's pools |
