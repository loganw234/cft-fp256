...
Thirty-seven files, 48,531 lines. Flat in one directory they look like one
...
- **1,743 lines of operations**, read when you are about to do something to
  hardware;
- **24,904 lines of record and argument** — the ledger, the roadmap and the
...
| document | lines | what it is for |
|---|---|---|
| [VALIDATION.md](VALIDATION.md) | 14,099 | The validation ledger, append-only. Every run, including the ones that failed and the mistakes that produced them. A figure here is a fact about the run on its date and is never edited to match a later one. |
