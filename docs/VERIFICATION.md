...
4. **The RTL, in simulation** (`tb/`, `make sim` in the `cft-sim`
   image). Twenty-three cocotb targets hold the hardware to the model, and
...
| `sim` (23 cocotb targets, `cft-sim` image) | 10 min at the runner's job count; about 40 min serial | **55 min at four jobs** | 3 min at twelve jobs on a 36-core box; almost all compilation |
