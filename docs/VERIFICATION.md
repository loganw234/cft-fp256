...
| `libcft` / `make -C host test` (build + the replay: 1,068,915 cases on `make vectors`' sets, about 1.2 million at the runner's generator counts), then `make -C host profiles-check` | 7.5 min | 8.5 to 11 min | the census was 1,071,635 until 2026-09-12, when opcode 31 became `maxall`: a reduction has no elementwise case to inherit, so 4,000 `reserved31` cases left and 1,280 maxall cases arrived. A document recording an earlier RUN still says 1,071,635 and is right to. `profiles-check` joined the stage on 2026-09-24 (19 s on the desktop, serial): the library's fifteen sources compiled with `-Werror=implicit-function-declaration` at the three reduced profiles `docs/EMBEDDED.md`'s loopback builds (`CFT_TINY`, `CFT_TINY` at fp128, the 32-bit boards') and at `CFT_NO_PROGRAM` alone. No runner stage had compiled any of them, and from 2026-09-14 until that day `divsqrt.c` did not compile with gcc 14 or later at `CFT_NO_PROGRAM`, and so not at either `CFT_TINY` profile |
