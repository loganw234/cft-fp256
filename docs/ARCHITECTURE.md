...
| 0xA8 | MASK_PTR | RW | 64-bit HBM byte address of the run's LANE MASK (ABI 0.14): `(n + 7) / 8` bytes, bit *i* lane *i*, set for a lane that runs; read at block setup through `m_axi_a` when MODE[23] is set, which CAPS2[10] announces and the same guard refuses without it. Argument 16. A masked lane runs nothing and writes nothing (docs/SEQUENCER.md, R17) |

These five sit **above** the read-only block rather than beside the
...
now runs to id 16 - `prog` 6, `cnt` 7, `bank` 8, `scratch_in` 9,
`scratch_out` 10, `seg` 11, and ABI 0.14's `idx_a` 12, `idx_b` 13,
`idx_c` 14, `idx_si` 15 and `mask` 16 - which is what the rows above
record.
...
- Read CAPS once at open: bit p set means MODE precision p is
  ...
  and CAPS[6] is what a host reads before it writes one. CAPS2 and the
  two scratch pointers are one step further still: VERSION 0x800, and
  CAPS2[4] and [5] are what a host reads before it uses the scratch or
  declares a block for it. SEG and NRES are VERSION 0x900 with CAPS2[8];
  the four index-table pointers and the mask pointer are VERSION 0xA00
  with CAPS2[9] and [10] - and a MODE bit the build cannot honour is
  refused at start (docs/SEQUENCER.md's MODE rule), so a host that
  reads the bits never reaches the refusal.
- **And read CAPS[27:16] and CAPS2 before building a program**, not
...
hw/link.cfg gives each master its own HBM pseudo-channel, and
...
time (`hw/mc_sweep.sh`, summaries in its `summary.txt`). These supersede
the 292/277 DSP pair in the geometry table near the top of this file,
which predates the seed ROM becoming case tables; the routed design's
FMA ladder reports **262** (the round-2 single-tile image reports 311 in
all: the ladder's 262, the integer multiply's 45 and four more -
docs/VALIDATION.md, 2026-09-16 and 2026-09-23):
