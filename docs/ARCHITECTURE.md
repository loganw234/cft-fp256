...
- Read CAPS once at open: bit p set means MODE precision p is
  ...
  and CAPS[6] is what a host reads before it writes one.
- **And read CAPS[27:16] before building a program**, not after the
  tile has refused one. Three four-bit exponents: deposit slots a
  lane, instructions, addressable constants. A host that skips this
  ...
  program to fit. All three read zero on a tile from before they
