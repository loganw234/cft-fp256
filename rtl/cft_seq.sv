...
module cft_seq #(
  ...
  generate
    for (genvar gw = 0; gw < WORDS; gw = gw + 1) begin : g_rf
      ...
      always_ff @(posedge ap_clk) begin
        rf_rdata_a[gw*32 +: 32] <= bank0[rf_raddr_a];
        rf_rdata_b[gw*32 +: 32] <= bank0[rf_raddr_b];
        rf_rdata_c[gw*32 +: 32] <= bank1[rf_raddr_c];
  ...
  // cycle.
  logic              db_we    [0:WORDS-1];
  logic [DBA-1:0]    db_waddr [0:WORDS-1];
  logic [31:0]       db_wdata [0:WORDS-1];
  logic [DBA-1:0]    db_raddr;
  logic [31:0]       db_rdata [0:WORDS-1];
  ...
  generate
    for (genvar gb = 0; gb < WORDS; gb = gb + 1) begin : g_db
      ...
      always_ff @(posedge ap_clk) begin
        if (db_we[gb])
          bank[db_waddr[gb]] <= db_wdata[gb];
        db_rdata[gb] <= bank[db_raddr];
      end
    end
  endgenerate

  // ---- lane state -----------------------------------------------------
  logic active [0:BLK_LANES-1];
  logic [$clog2(MAXD+1)-1:0] dcnt [0:BLK_LANES-1];
  logic any_active;
  always_comb begin
    any_active = 1'b0;
  ...
  always_comb rd_bl = burst_len(rd_addr, rd_beats_left);
  always_comb wr_bl = burst_len(wr_addr, wr_beats_left);
  ...
  typedef enum logic [4:0] {
    ...
    S_SET_RD, S_SET_AP,
    S_SKIP_F, S_SKIP_D,
    S_DRAIN_SETUP, S_DRAIN_RD, S_DRAIN_PACK, S_DRAIN_SEND,
  ...
  function automatic logic lane_mag_nz(input [BEAT_BITS-1:0] v,
    ...
    begin
      s = v >> ({3'b0, posn} << ({3'b0, wpe_sh} + 5));  // posn*esz*8
      case (prec_q)
  ...
  // the OR of the writeback beat's active-lane flags, ready for one
  // sticky update in the state machine
  logic [4:0] wb_flags_or;
  always_comb begin
    wb_flags_or = 5'b0;
    for (int p = 0; p < WORDS; p = p + 1)
      if (p < 32'(lpb) && (32'(wb_bt) << lpb_sh) + p < BLK_LANES &&
          active[(32'(wb_bt) << lpb_sh) + p])
        wb_flags_or = wb_flags_or | al_lf[p*5 +: 5];
  end

  logic [255:0] drain_elem;
  always_comb begin
    logic [2:0] posn;
    posn = 3'(lane_cursor[LB-1:0] & (lpb - 1));
    drain_elem = '0;
    for (int w = 0; w < WORDS; w = w + 1)
      if (w < (32'd1 << wpe_sh))
        drain_elem[w*32 +: 32] =
          db_rdata[32'({29'b0, posn} << wpe_sh) + w];
    if (slot_cursor >= 32'(dcnt[lane_cursor[LB-1:0]]))
      drain_elem = '0;
  end
  ...
  always_ff @(posedge ap_clk) begin
    if (!ap_rst_n) begin
      ...
      for (int p = 0; p < WORDS; p = p + 1) db_we[p] <= 1'b0;
      ...
      if (m_wr_bvalid && m_wr_bready) begin
        if (m_wr_bresp != 2'b00) wr_fault_q <= 1'b1;
      ...
      case (st)
        ...
        S_CHECK: begin
          ...
          h_nconsts <= hdr[3];
          h_maxdep  <= hdr[4];
          if (hdr[0] != 32'h5054_4643 || hdr[1] != 32'd1 ||
              hdr[5] != {30'b0, prec_q} ||
              hdr[2] > IMEM_D || hdr[3] > KMEM_D || hdr[4] > MAXD) begin
            refuse_q <= 1'b1;
            st <= S_FIN;
          end else
            st <= S_IMG_GO;
        ...
        S_IMG_PARSE: begin
          ...
          // rready tracks window room so a beat never arrives with
          // nowhere to go.
          ...
          m_rd_rready <= (pw_have <= 7'(BEAT_BYTES)) &&
                         !(kons_left == 0 && insn_left == 0);
        end
        ...
        S_BLK_SETUP: begin
          if (blk_base >= n_q)
            ...
            blk_n <= (n_q - blk_base > 64'(BLK_LANES))
                     ? (LB+1)'(BLK_LANES)
                     : (LB+1)'(n_q - blk_base);
            zaddr <= '0;
            st <= S_ZERO;
        ...
        S_ZERO: begin
          ...
          if (zaddr == 9'(RF_D - 1)) begin
            for (int l = 0; l < BLK_LANES; l = l + 1) begin
              active[l] <= (blk_base + l < n_q);
              dcnt[l] <= '0;
            end
        ...
        S_DECODE: begin
          if (!c_ctrl) begin
            bt <= '0; wb_bt <= '0;
            rf_raddr_a <= {c_ra, 4'b0};
            rf_raddr_b <= {c_rb, 4'b0};
            rf_raddr_c <= {c_rc, 4'b0};
            st <= S_ALU_ISSUE;
          end else begin
            case (c_op)
              C_REPEAT: begin
        ...
        // ---- ALU issue / writeback ------------------------------------
        S_ALU_ISSUE: begin
          // addresses run one beat ahead of the array: beat bt's
          // operands were addressed last cycle and are on rf_rdata_*
          // now
          rf_raddr_a <= {c_ra, bt[3:0] + 4'd1};
          rf_raddr_b <= {c_rb, bt[3:0] + 4'd1};
          rf_raddr_c <= {c_rc, bt[3:0] + 4'd1};
          al_valid <= 1'b1;
          al_op <= c_op;
          al_rnd <= c_rnd;
        ...
        S_DEP_WR: begin
          ...
          for (int p = 0; p < WORDS; p = p + 1)
            if ((p >> wpe_sh) < 32'(lpb) &&
                ...
                32'(dcnt[(32'(bt) << lpb_sh) + (p >> wpe_sh)])
                  < h_maxdep) begin
              db_we[p]    <= 1'b1;
              db_waddr[p] <= DBA'(32'(bt) * MAXD +
                  32'(dcnt[(32'(bt) << lpb_sh) + (p >> wpe_sh)]));
              db_wdata[p] <= rf_rdata_a[p*32 +: 32];
            end
          for (int l = 0; l < BLK_LANES; l = l + 1)
            if ((l >> lpb_sh) == 32'(bt) && active[l]) begin
              if (32'(dcnt[l]) < h_maxdep)
                dcnt[l] <= dcnt[l] + 1;
              else
                dep_ovf_q <= 1'b1;
            end
        ...
        S_NEXT_BLK: begin
          blk_base <= blk_base + 64'(BLK_LANES);
          st <= S_BLK_SETUP;
        end
