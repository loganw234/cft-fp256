...
| a sequencer to run a chain on-chip | **benched, and failing on a device** (2026-09-02) - `cft_seq` exists in RTL, driving the kernel's one ALU array (shared with the streaming engine since 2026-09-01; its private copy had tripled the tile), and holds bit-exact to `seq.py` on deposits, counts, flags and status: 9/9 unit-bench suites across all four formats (single-op through nested-loop escape maps, ragged blocks, a 62-program fuzz corpus) plus the full-kernel bench through the CSR. **Then hw_emu ran it and every program failed** - four of four, each reporting STATUS[2:0], the bus-fault bits - while the elementwise engine passed on the same image in the same session. So the semantics are proven against the model and the BUS BEHAVIOUR is not: the cocotb benches drive cocotbext-axi's AxiRam, which does not push back the way an interconnect does. Bank binding, argument binding and the 4 KB burst rule are ruled out by inspection (docs/VALIDATION.md, 2026-09-02); the root cause is open. Do not read this row as a working on-chip sequencer |
